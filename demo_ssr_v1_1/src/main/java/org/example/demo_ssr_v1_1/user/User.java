...
public class User {
    ...
    /**
     * User (1): UserRole(N)
     * User 가 UserRole 리스트를 관리(단방향)
     * 실제 DB의 'user_role_tb' 테이블에 user_id 라는 fk 컬럼이 생김
     *
     * CascadeType.ALL
     * - 운명 공동체 User 를 저장하면 Role로 자동 저장되고,
     * User를 삭제하면 가지고 있던 Role 들도 다 같이 삭제
     * EX) 홍길동 - 관리자, 일반 사용자 : 삭제하면 userRole 2가지 row도 자동 삭제
     *
     * orphanRemoval = true
     * 리스와 DB의 동기화
     * Java의 roles 리스에서 요소(role)를 .remove() 하거나 .clear() 하면
     * DB에서도 해당 데이터(DELETE)가 실제로 처리
     */
    // 나중에 다른 개발자가 findById(쿼리 메서드 호출할 때 신경 쓸 필요없이 전부 role 까지 반환)
    ...
    public boolean hasRole(Role role) {
        ...
        // any(어떤것이든), Match(일치하다) 즉, 리스트 안에 있는 것들 중 하나라도 조건이 맞는게 있다면 true 반환
    ...
    // 분기처리 (머스태치 화면에서는 서버에 저장된 이미지든, URL 이미지 이든
    //      그냥 getProfilePath 변수를 호출하면 알아서 셋팅 되게 하고 싶다)
    ...
    public void chargePoint(Integer amount) {
        if (amount == null || amount <= 0) {
            throw new Exception400("포인트는 0보다 커야 합니다.");
        }
